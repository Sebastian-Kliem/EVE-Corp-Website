            ...
            return array_map('intval', $this->connection->fetchFirstColumn(
                "SELECT t.typeID FROM invTypes t JOIN invGroups g ON t.groupID = g.groupID 
                 WHERE g.categoryID = 25 
                    ...
                    OR t.typeID IN (34, 33577, 33539, 33521, 33527, 33536, 33543, 33545, 33546, 33547, 33548, 33556, 33558, 33560, 33562, 33564, 33566, 57442, 57443, 57444, 57445, 57446, 57447, 57448, 57449, 57450, 57451, 57452)"
            ));
        } catch (\Exception $e) {
            return [];
        }
